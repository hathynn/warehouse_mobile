...
export default function ReceiptDetail() {
  ...
  const { importOrders, loading, fetchImportOrders } = useImportOrder();
          ...
          <View className="bg-black px-4 py-4 flex-row justify-between items-center rounded-2xl">
            ...
            {filteredOrders.length > 0 && (
              <Text className="text-white font-bold text-lg">
                Đơn nhập số{" "}
                <Text className="text-blue-200">#{filteredOrders[0].id}</Text>
              </Text>
            )}
          </View>
            ...
            filteredOrders.map((order: any) => (
              <TouchableOpacity>
                ...
                >
                  ...
                  <Text>
                    Warehouse Keeper: {order.warehouse_keeper_assigned}
                  </Text>
                  ...
                  marginTop="20"
                    onPress={() =>
                      router.push(`/import/create-import/${order.id}`)
                    }
                  >
                    Tạo chứng từ
                  </Button>
